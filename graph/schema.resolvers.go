package graph

// This file will be automatically regenerated based on the schema, any resolver implementations
// will be copied through when generating and any unknown code will be moved to the end.
// Code generated by github.com/99designs/gqlgen version v0.17.78

import (
	"context"
	"fmt"
	"log"
	"sdmht-server/db"
	"sdmht-server/game"
	"sdmht-server/graph/model"
	"sdmht-server/models"
	"time"

	"github.com/olebedev/emitter"
)

// AddSubscription is the resolver for the addSubscription field.
func (r *mutationResolver) AddSubscription(ctx context.Context, subscription model.PushSubscription) (*Void, error) {
	// 转换graphql模型到数据库模型
	dbSubscription := &models.PushSubscription{
		Endpoint: subscription.Endpoint,
		P256dh:   subscription.Keys.P256dh,
		Auth:     subscription.Keys.Auth,
	}

	// 处理过期时间
	if subscription.ExpirationTime != nil {
		timestamp := int64(*subscription.ExpirationTime)
		parsedTime := time.Unix(timestamp/1000, (timestamp%1000)*1000000)
		dbSubscription.ExpirationTime = &parsedTime
	}

	// 在数据库中创建或更新订阅记录
	result := db.DB.Where("endpoint = ?", subscription.Endpoint).Assign(*dbSubscription).FirstOrCreate(dbSubscription)
	if result.Error != nil {
		return nil, fmt.Errorf("failed to save subscription: %v", result.Error)
	}

	return nil, nil
}

// Time is the resolver for the time field.
func (r *queryResolver) Time(ctx context.Context) (*time.Time, error) {
	now := time.Now()
	return &now, nil
}

// MatchOpponent is the resolver for the matchOpponent field.
func (r *subscriptionResolver) MatchOpponent(ctx context.Context, uid string, size int32, version string) (<-chan any, error) {
	log.Print(uid, " 匹配")
	ch := make(chan any, 1)
	topic := "send_data" + uid
	off := r.game.Event.On(topic, func(e *emitter.Event) {
		ch <- e.Args[0]
	})
	r.game.Pmu.Lock()
	r.game.Player[uid] = game.Player{
		Size:    size,
		Version: version,
	}
	go func() {
		defer func() {
			r.game.Pmu.Lock()
			delete(r.game.MatchingPlayer, uid)
			delete(r.game.Player, uid)
			r.game.Pmu.Unlock()
			r.game.Event.Off(topic, off)
			close(ch)
		}()
		is_matched := false
		for o_uid, p := range r.game.MatchingPlayer {
			if p.Size == size && p.Version == version && o_uid != uid {
				log.Print(uid, " 匹配到 ", o_uid)
				delete(r.game.MatchingPlayer, o_uid)
				go r.game.Event.Emit("send_data"+o_uid, uid)
				ch <- ""
				ch <- o_uid
				is_matched = true
				break
			}
		}
		if !is_matched {
			log.Print(uid, " 匹配等待中")
			r.game.MatchingPlayer[uid] = r.game.Player[uid]
		}
		r.game.Pmu.Unlock()
		<-ctx.Done()
		log.Print(uid, " 匹配结束")
	}()
	return ch, nil
}

// SendData is the resolver for the sendData field.
func (r *subscriptionResolver) SendData(ctx context.Context, to string, data any) (<-chan *Void, error) {
	log.Print("向 ", to, " 发送数据：", data)
	ch := make(chan *Void, 1)
	defer close(ch)
	go r.game.Event.Emit("send_data"+to, data)
	return ch, nil
}

// Heartbeat is the resolver for the heartbeat field.
func (r *subscriptionResolver) Heartbeat(ctx context.Context, uid string) (<-chan *Void, error) {
	ch := make(chan *Void, 1)
	r.game.Join(uid)
	go func() {
		defer r.game.Leave(uid)
		defer close(ch)
		<-ctx.Done()
	}()
	return ch, nil
}

// ListenAlive is the resolver for the listenAlive field.
func (r *subscriptionResolver) ListenAlive(ctx context.Context, uid string) (<-chan *Void, error) {
	ch := make(chan *Void, 1)
	go func() {
		defer close(ch)
		if !r.game.IsOnline(uid) {
			ch <- nil
			return
		}
		topic := "leave" + uid
		off := r.game.Event.On(topic, func(e *emitter.Event) {
			ch <- nil
		})
		defer r.game.Event.Off(topic, off)
		<-ctx.Done()
	}()
	return ch, nil
}

// OnlineCount is the resolver for the onlineCount field.
func (r *subscriptionResolver) OnlineCount(ctx context.Context) (<-chan int32, error) {
	ch := make(chan int32, 1)
	ch <- r.game.OnlineCount()
	go func() {
		defer close(ch)
		off := r.game.Event.On("online_changed", func(e *emitter.Event) {
			ch <- e.Args[0].(int32)
		})
		defer r.game.Event.Off("online_changed", off)
		<-ctx.Done()
	}()
	return ch, nil
}

// Time is the resolver for the time field.
func (r *subscriptionResolver) Time(ctx context.Context) (<-chan *time.Time, error) {
	ch := make(chan *time.Time, 1)
	ticker := time.NewTicker(1 * time.Second)
	go func() {
		defer ticker.Stop()
		defer close(ch)
		now := time.Now()
		ch <- &now
		for {
			select {
			case <-ticker.C:
				now = time.Now()
				ch <- &now
			case <-ctx.Done():
				return
			}
		}
	}()
	return ch, nil
}

// Mutation returns MutationResolver implementation.
func (r *Resolver) Mutation() MutationResolver { return &mutationResolver{r} }

// Query returns QueryResolver implementation.
func (r *Resolver) Query() QueryResolver { return &queryResolver{r} }

// Subscription returns SubscriptionResolver implementation.
func (r *Resolver) Subscription() SubscriptionResolver { return &subscriptionResolver{r} }

type mutationResolver struct{ *Resolver }
type queryResolver struct{ *Resolver }
type subscriptionResolver struct{ *Resolver }
